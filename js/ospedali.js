const NOMI_TABELLA = ["Ospedali", "ospedali"];
...
function messaggioTabella(error) {
  if (tabellaNonTrovata(error)) {
    return "La tabella Ospedali c'è, ma Supabase non l'ha ancora messa nell'elenco dell'app. In SQL Editor esegui sql/ospedali-app.sql, aspetta 10 secondi e ricarica questa pagina.";
  }
...
function etichettaForm(nome) {
  if (eColonnaLink(nome)) {
    return "Link (Google Maps)";
  }
  return String(nome).replace(/_/g, " ");
}
...
function eColonnaNascosta(nome) {
  ...
  return basso === "id" || basso === "created_at" || basso === "user_id" || basso === "idospedale";
}

function eColonnaLink(nome) {
  const basso = String(nome).toLowerCase();
  return basso === "link" || basso === "url" || basso === "maps" || basso === "mappa";
}

function colonnaId() {
  return trovaColonna("id") || trovaColonna("IdOspedale") || trovaColonna("IDOSPEDALE") || "id";
}
...
function idDellaRiga(riga) {
  ...
  const col = colonnaId();
  const candidati = [riga[col], riga.IDOSPEDALE, riga.IdOspedale, riga.idospedale, riga.id, riga.Id];
...
function colonnaLuogo() {
  return trovaColonna("LUOGO") || trovaColonna("Luogo") || trovaColonna("luogo");
}

function colonnaNome() {
  return (
    trovaColonna("Nome") ||
    trovaColonna("Ospedale") ||
    trovaColonna("NOMINATIVO") ||
    trovaColonna("Titolo")
  );
}
...
function colonneTabella() {
  ...
  return colonne.filter(function (col) {
    return !eColonnaNascosta(col) && col !== colonnaLuogo();
  }).slice(0, 1);
}
...
function tipoCampo(nome) {
  ...
  if (eColonnaLink(nome) || /maps|mappa|^url$/.test(basso)) {
    return "text";
  }
  if (/telefono|tel|cellulare|phone/.test(basso)) {
    return "tel";
  }
...
function usaCombo(nome) {
  const basso = String(nome).toLowerCase();
  return /luogo|citta|città|tipo|zona/.test(basso);
}
...
function valoreLuogo(riga) {
  const col = colonnaLuogo();
  if (!col) {
    return "(senza luogo)";
  }
...
function urlNavigazione(valore) {
  ...
  if (/^(www\.|maps\.|goo\.gl|maps\.app)/i.test(testo)) {
    return "https://" + testo;
  }
  return "";
}

function creaBottoneNaviga(url) {
  const bottone = document.createElement("a");
  bottone.className = "bottone-naviga";
  bottone.href = url;
  bottone.target = "_blank";
  bottone.rel = "noopener noreferrer";
  bottone.textContent = "Naviga";
...
function righeVisibili() {
  ...
  const colNome = colonnaNome();
  return elenco.slice().sort(function (a, b) {
    const perLuogo = confrontaTesto(valoreLuogo(a), valoreLuogo(b));
    if (perLuogo !== 0) {
      return perLuogo;
    }
    if (!colNome) {
...
function raggruppaPerLuogo(righe) {
  const gruppi = {};
  righe.forEach(function (riga) {
    const nomeGruppo = valoreLuogo(riga);
...
function applicaChiusure(forzaAperti) {
  corpoTabella.querySelectorAll("[data-luogo]").forEach(function (riga) {
    const nomeGruppo = riga.getAttribute("data-luogo");
  ...
  corpoTabella.querySelectorAll(".riga-gruppo button").forEach(function (bottone) {
    const nomeGruppo = bottone.closest("tr").getAttribute("data-luogo");
...
function costruisciCampi(riga) {
  ...
  modificabili.forEach(function (nome) {
    ...
    span.textContent = etichettaForm(nome);
    const tipo = tipoCampo(nome);
...
function disegnaIntestazione() {
  ...
  colonneTabella().forEach(function (nome) {
    ...
    if (eColonnaLink(nome)) {
      th.className = "cella-naviga";
    } else {
      th.className = "cella-nome";
    }
...
function disegnaTabella() {
  ...
  if (cacheRighe.length === 0) {
    ...
    td.textContent =
      "Nessuna riga visibile. Se in Table Editor ci sono dati, esegui sql/ospedali-app.sql.";
    tr.appendChild(td);
  ...
  const gruppi = raggruppaPerLuogo(visibili);
  const luoghi = Object.keys(gruppi).sort(confrontaTesto);
  const numColonne = Math.max(colonneTabella().length, 1);

  luoghi.forEach(function (nomeGruppo) {
    const trGruppo = document.createElement("tr");
    trGruppo.className = "riga-gruppo";
    trGruppo.setAttribute("data-tipo", "gruppo");
    trGruppo.setAttribute("data-luogo", nomeGruppo);
    ...
    gruppi[nomeGruppo].forEach(function (riga) {
      ...
      tr.setAttribute("data-luogo", nomeGruppo);
      colonneTabella().forEach(function (nome) {
        const td = document.createElement("td");
        if (eColonnaLink(nome)) {
          td.className = "cella-naviga";
          const url = urlNavigazione(riga[nome]);
          if (url) {
            td.appendChild(creaBottoneNaviga(url));
          } else {
            td.textContent = "—";
          }
        } else {
...
corpoTabella.addEventListener("click", function (evento) {
  ...
  if (bottoneGruppo) {
    const nomeGruppo = bottoneGruppo.closest("tr").getAttribute("data-luogo");
    chiusiGruppo[nomeGruppo] = gruppoChiuso(nomeGruppo) ? false : true;
    applicaChiusure(!!testoRicerca());
    return;
  }

  if (evento.target.closest(".bottone-naviga")) {
    return;
  }
