const NOMI_TABELLA = ["MemorieVarie", "memorievarie", "Memorie_Varie", "memorie_varie"];
...
function messaggioTabella(error) {
  if (tabellaNonTrovata(error)) {
    return "La tabella MemorieVarie c'è, ma Supabase non l'ha ancora messa nell'elenco dell'app. In SQL Editor esegui sql/memorie-varie-app.sql, aspetta 10 secondi e ricarica questa pagina.";
  }
...
function colonnaGenere() {
  return trovaColonna("GENERE") || trovaColonna("Genere") || trovaColonna("genere");
}

function valoreGenere(riga) {
  const col = colonnaGenere();
  if (!col) {
    return "(senza genere)";
  }
...
function genereChiuso(nomeGruppo) {
  return chiusiGenere[nomeGruppo] !== false;
}

function colonneTabella() {
  const visibili = colonne.filter(function (nome) {
    return !eColonnaNascosta(nome) && nome !== colonnaGenere();
  });
...
function tipoCampo(nome) {
  ...
  if (/note|descrizione|commento|testo|memoria|ricordo|contenuto|dettaglio/.test(basso)) {
    return "textarea";
  }
  return "text";
}
...
function usaCombo(nome) {
  const basso = String(nome).toLowerCase();
  return /tipo|categoria|luogo|persona|argomento|tag|genere/.test(basso);
}
...
function righeVisibili() {
  ...
  const colData = colonnaData() || trovaColonna("created_at");
...
function raggruppaPerGenere(righe) {
  const gruppi = {};
  righe.forEach(function (riga) {
    const nomeGruppo = valoreGenere(riga);
...
function applicaChiusure(forzaAperti) {
  corpoTabella.querySelectorAll("[data-genere]").forEach(function (riga) {
    const nomeGruppo = riga.getAttribute("data-genere");
    const tipo = riga.getAttribute("data-tipo");
    const nascosta = !forzaAperti && tipo === "dati" && genereChiuso(nomeGruppo);
    riga.classList.toggle("riga-nascosta", nascosta);
  });

  corpoTabella.querySelectorAll(".riga-gruppo button").forEach(function (bottone) {
    const nomeGruppo = bottone.closest("tr").getAttribute("data-genere");
    const aperto = forzaAperti || !genereChiuso(nomeGruppo);
...
function disegnaTabella() {
  ...
  if (cacheRighe.length === 0) {
    ...
    td.textContent =
      "Nessuna riga visibile. Se in Table Editor ci sono dati, esegui sql/memorie-varie-app.sql.";
    tr.appendChild(td);
  ...
  const gruppi = raggruppaPerGenere(visibili);
  const generi = ordinaGeneri(Object.keys(gruppi));
  const numColonne = Math.max(colonneTabella().length, 1);

  generi.forEach(function (nomeGruppo) {
    const trGruppo = document.createElement("tr");
    trGruppo.className = "riga-gruppo";
    trGruppo.setAttribute("data-tipo", "gruppo");
    trGruppo.setAttribute("data-genere", nomeGruppo);
    ...
    gruppi[nomeGruppo].forEach(function (riga) {
      ...
      tr.setAttribute("data-genere", nomeGruppo);
      colonneTabella().forEach(function (nome) {
        const td = document.createElement("td");
...
corpoTabella.addEventListener("click", function (evento) {
  ...
  if (bottoneGruppo) {
    const nomeGruppo = bottoneGruppo.closest("tr").getAttribute("data-genere");
    chiusiGenere[nomeGruppo] = genereChiuso(nomeGruppo) ? false : true;
    applicaChiusure(!!testoRicerca());
    return;
  }
