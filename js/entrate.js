const NOMI_TABELLA = ["Entrate", "entrate"];
...
function messaggioTabella(error) {
  if (tabellaNonTrovata(error)) {
    return "La tabella Entrate c'è, ma Supabase non l'ha ancora messa nell'elenco dell'app. In SQL Editor esegui sql/entrate-app.sql, aspetta 10 secondi e ricarica questa pagina.";
  }
...
function eColonnaNascosta(nome) {
  ...
  return basso === "id" || basso === "created_at" || basso === "user_id" || basso === "identrate";
}

function colonnaId() {
  return trovaColonna("IDENTRATE") || trovaColonna("IdEntrata") || trovaColonna("id") || "id";
}
...
function idDellaRiga(riga) {
  ...
  const col = colonnaId();
  const candidati = [riga[col], riga.IDENTRATE, riga.identrate, riga.IdEntrata, riga.id, riga.Id];
...
function colonnaImporto() {
  const daEntrata = trovaColonna("entrata") || trovaColonna("Entrata") || trovaColonna("importo") || trovaColonna("Importo");
  if (daEntrata) {
    nomeColonnaImporto = daEntrata;
    return daEntrata;
  }
  ...
  const daNome = colonne.find(function (nome) {
    const basso = String(nome).toLowerCase();
    return /importo|^imp$|^entrata$|^spesa$|euro|prezzo|costo|totale|valore|soldi|cifra/.test(basso);
  });
  if (daNome) {
...
function colonnaImportoDaValori() {
  ...
  colonne.forEach(function (nome) {
    if (eColonnaNascosta(nome) || nome === id || nome === data) {
      return;
    }
...
function tipoCampo(nome) {
  ...
  if (/importo|^imp$|^entrata$|^spesa$|prezzo|euro|valore|soldi|cifra|litri|km|chilometr|quantit|numero|costo/.test(basso)) {
    return "number";
  }
...
function usaCombo(nome) {
  const basso = String(nome).toLowerCase();
  return /tipo|categoria|fonte|cliente|provenienza|pagamento|metodo|voce|sezione/.test(basso);
}
...
function disegnaTabella() {
  ...
  if (cacheRighe.length === 0) {
    ...
    td.textContent =
      "Nessuna riga visibile. Se in Table Editor ci sono dati, esegui sql/entrate-app.sql.";
    tr.appendChild(td);
  ...
  anni.forEach(function (anno) {
    const righeAnno = gruppi[anno].slice().sort(function (a, b) {
      if (!colData) {
        return 0;
      }
      return isoDaValoreData(b[colData]).localeCompare(isoDaValoreData(a[colData]));
