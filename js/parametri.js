const NOMI_TABELLA = ["Parametri", "parametri"];
...
function messaggioTabella(error) {
  if (tabellaNonTrovata(error)) {
    return "La tabella Parametri c'è, ma Supabase non l'ha ancora messa nell'elenco dell'app. In SQL Editor esegui sql/parametri-app.sql, aspetta 10 secondi e ricarica questa pagina.";
  }
...
function eColonnaNascosta(nome) {
  ...
  return (
    ...
    basso === "user_id" ||
    compatto === "id" ||
    compatto === "idparametro" ||
    compatto === "idparametri" ||
    (colonnaId() && nome === colonnaId() && compatto.indexOf("id") === 0)
  );
}

function colonnaId() {
  const preferite = ["id", "idparametri", "idparametro"];
  ...
  const iniziaConId = colonne.find(function (nome) {
    const compatto = nomeColonnaCompatto(nome);
    return compatto.indexOf("id") === 0 && compatto !== "userid";
  });
  if (iniziaConId) {
    return iniziaConId;
  }
...
function isoDaValoreData(valore) {
  if (valore instanceof Date && !isNaN(valore.getTime())) {
    return (
      valore.getFullYear() +
      "-" +
      dueCifre(valore.getMonth() + 1) +
      "-" +
      dueCifre(valore.getDate())
    );
  }
  ...
  if (trovato) {
    return trovato[3] + "-" + dueCifre(trovato[2]) + "-" + dueCifre(trovato[1]);
  }
  return "";
}
...
function colonnaSembraNumerica(nome) {
  let visti = 0;
  let numerici = 0;
  cacheRighe.forEach(function (riga) {
    ...
    if (/^-?\d+(\.\d+)?$/.test(testo) && Number.isFinite(Number(testo))) {
      numerici += 1;
    }
  });
  return visti > 0 && numerici === visti;
}
...
function tipoCampo(nome) {
  ...
  if (
    ...
    basso === "minimo" ||
    basso === "massimo" ||
    /importo|prezzo|euro|valore|dose|quantit|numero|costo|peso|pressione|glicemia|risultato|riferiment|sistol|diastol|hdl|ldl|colesterol|triglicer|^bmi$|altezza|freq|battit|temperat|saturaz/.test(
      basso
    ) ||
    colonnaSembraNumerica(nome)
  ) {
    return "number";
  }
  if (/note|descrizione|commento|testo|dettaglio/.test(basso)) {
    return "textarea";
  }
  return "text";
}
...
function usaCombo(nome) {
  const basso = String(nome).toLowerCase();
  return /tipo|parametro|esame|unita|unita'|categoria|nome|misura/.test(basso);
}
...
function confrontaPerData(a, b) {
  const colData = colonnaData();
  if (!colData) {
    return 0;
  }
  const isoA = isoDaValoreData(a[colData]);
  const isoB = isoDaValoreData(b[colData]);
...
function righeVisibili() {
  ...
  return elenco.slice().sort(confrontaPerData);
}
...
function leggiForm() {
  ...
  colonneModificabili().forEach(function (nome) {
    ...
    if (tipo === "number") {
      payload[nome] = valoreNumerico(testo);
    } else if (tipo === "datetime-local") {
      payload[nome] = testo.replace("T", " ") + ":00";
    } else {
      payload[nome] = testo;
    }
  });
...
function apriDialogo(riga, modificaSubito) {
  ...
  campoId.value = idTrovato !== "" && idTrovato !== null && idTrovato !== undefined ? String(idTrovato) : "";
...
function disegnaTabella() {
  ...
  if (cacheRighe.length === 0) {
    ...
    td.textContent =
      "Nessuna riga visibile. Se in Table Editor ci sono dati, esegui sql/parametri-app.sql.";
    tr.appendChild(td);
  ...
  anni.forEach(function (anno) {
    ...
    gruppi[anno].slice().sort(confrontaPerData).forEach(function (riga) {
      const tr = document.createElement("tr");
